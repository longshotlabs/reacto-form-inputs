...
testInput({
  ...
  simulateChanging(wrapper, value) {
    const { dayValue, monthValue, timeValue, yearValue } = getDateTimeValuesFromDate(
      value,
      moment,
      timezone
    )
    ...
    wrapper
      .find('input')
      .at(0)
      .simulate('change', { target: { value: dayValue } })
    wrapper
      .find('input')
      .at(1)
      .simulate('change', { target: { value: yearValue } })
    wrapper
      .find('input')
      .at(2)
      .simulate('change', { target: { value: timeValue } })
  },
...
test('renders', () => {
  const component = renderer.create(<DateTimeInput name='test' moment={moment} />)
